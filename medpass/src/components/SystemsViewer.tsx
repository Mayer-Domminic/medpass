...
const SystemsViewer: React.FC<SystemsViewerProps> = ({
  ...
  return (
    <div className={`${className}`}>
      <div className="h-[500px] border border-gray-700/50 rounded p-4 bg-gray-800/50 w-full">
        <div className="relative h-full">
          ...
          {/* System name indicators for highlighted systems */}
          {highlightedSystems.length > 0 && (
            <div className="absolute bottom-2 left-0 right-0 text-center text-white bg-black/30 py-1 rounded">
              {highlightedSystems.length === 1 
                ? `${highlightedSystems[0].charAt(0).toUpperCase() + highlightedSystems[0].slice(1)} System`
                : `${highlightedSystems.length} Systems Highlighted`}
            </div>
          )}
        </div>
      </div>
