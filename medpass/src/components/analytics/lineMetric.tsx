...
const LineMetric: React.FC<LineMetricProps> = ({
  title,
  value,
  trend,
  color,
  ...
  return (
    <div className="relative">
      <Card
        className={`bg-gray-800/50 border-gray-700 transition-all duration-200 ${isHovered ? 'shadow-md shadow-' + color.replace('#', '') + '/30 border-' + color.replace('#', '') + '/40 scale-[1.02]' : ''
          ...
          <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <p className="text-medium font-semibold text-gray-400">{title}</p>
                {description && (
                  <span
              ...
              {/* Only render trend if it's not zero */}
              {trend !== 0 && (
